...
void DataFlash_Class::Log_Write_CameraInfo(enum LogMessages msg, const AP_AHRS &ahrs, const AP_Mount& mount, const AP_GPS &gps, const Location &current_loc)
{
		Location mount_location = mount.location(0, current_loc, ahrs);
    int32_t altitude, altitude_rel, altitude_gps;
    if (mount_location.flags.relative_alt) {
        altitude = mount_location.alt+ahrs.get_home().alt;
        altitude_rel = mount_location.alt;
    } else {
        altitude = mount_location.alt;
        altitude_rel = mount_location.alt - ahrs.get_home().alt;
    }
    ...
    struct log_Camera pkt = {
        ...
        latitude    : mount_location.lat,
        longitude   : mount_location.lng,
        altitude    : altitude,
...
// Write a Camera packet
void DataFlash_Class::Log_Write_Camera(const AP_AHRS &ahrs, const AP_Mount& mount, const AP_GPS &gps, const Location &current_loc)
{
    Log_Write_CameraInfo(LOG_CAMERA_MSG, ahrs, mount, gps, current_loc);
}

// Write a Trigger packet
void DataFlash_Class::Log_Write_Trigger(const AP_AHRS &ahrs, const AP_Mount& mount, const AP_GPS &gps, const Location &current_loc)
{
    Log_Write_CameraInfo(LOG_TRIGGER_MSG, ahrs, mount, gps, current_loc);
}
